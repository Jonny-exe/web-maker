...
export const SaveProject = (
	...
	useEffect(() => {
		if (sentCount !== 0) {
			...
			setState((state) => ({
				responseSavedStatus: state.responseSavedStatus,
				loadingSaved: true,
			}))
...
export const CreateProjectTokenRecovery = (
	recoverykey: string,
	createCount: number,
) => {
	const [state, setState] = useState({
		responseToken: "",
		loadingResponseToken: true,
	})
...
export const GetTokenFromRecovery = (
	recoverykey: string,
	...
	useEffect(() => {
		if (
			...
			setState((state) => ({
				responseTokenFromRecovery: state.responseTokenFromRecovery,
				loading: true,
			}))
			const loadData = async () => {
				const response = await fetch(thisURL, requestOptions)
...
export const RemoveFile = (token: string, removeFileCount: number) => {
	const [state, setState] = useState({
		removeFileStatus: 0,
		loadingRemoveFile: true,
	})
	const thisURL = url + "removeFile"
	useEffect(() => {
		if (
			removeFileCount > 0 &&
			removeFileCount !== null &&
			removeFileCount !== undefined
		) {
			var bodyContent = {
				token: token,
			}
			...
			setState((state) => ({
				removeFileStatus: state.removeFileStatus,
				loadingRemoveFile: true,
			}))
			const loadData = async () => {
				const response = await fetch(thisURL, requestOptions)
...
export const CheckRecoveryKey = (
	recoveryKey: string,
	checkRecoveryKeyCount: number,
) => {
	const [state, setState] = useState({
		checkRecoveryKeyStatus: 0,
		loadingCheckRecoveryKey: true,
	})
	const thisURL = url + "doesRecoveryKeyExist"
	useEffect(() => {
		if (
			checkRecoveryKeyCount > 0 &&
			checkRecoveryKeyCount !== null &&
			checkRecoveryKeyCount !== undefined
		) {
			var bodyContent = {
				recovery_key: recoveryKey,
			}
			...
			setState((state) => ({
				checkRecoveryKeyStatus: state.checkRecoveryKeyStatus,
				loadingCheckRecoveryKey: true,
			}))
			const loadData = async () => {
				const response = await fetch(thisURL, requestOptions)
				var data: number = await response.json()
				setState({
					checkRecoveryKeyStatus: data,
					loadingCheckRecoveryKey: false,
				})
			}
			loadData()
		} else {
